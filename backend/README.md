...
- `POST /api/violations` — proctoring client (frontend TensorFlow.js/face-api.js + fullscreen/visibility listeners) posts events here; weighted violation counting (phone/multi-face count double) auto-kicks and force-submits the attempt at the threshold. `GET /api/violations/:attemptId` is the trainer/org-admin audit trail.
