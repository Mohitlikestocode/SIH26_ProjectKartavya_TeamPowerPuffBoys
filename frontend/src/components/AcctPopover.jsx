    ...
    <>
      ...
      <div role="dialog" aria-label="Account" style={css("position:fixed; top:92px; right:24px; width:min(92vw,300px); background:#fff; border:1px solid #C9CFD8; box-shadow:0 10px 30px rgba(10,34,64,0.20); z-index:65; border-radius:4px; padding:18px")}>
        <div style={css("display:flex; gap:12px; align-items:center")}>
          <div style={css(`width:40px; height:40px; border-radius:3px; background:${v.roleColor}; color:#fff; display:flex; align-items:center; justify-content:center; font-size:14px; font-weight:700; flex-shrink:0`)}>{v.initials}</div>
          <div style={css("min-width:0")}>
            <div style={css("font-size:14.5px; font-weight:700; color:#0A2240; line-height:1.3")}>{v.userName}</div>
        ...
        <div style={css("display:flex; gap:8px; align-items:center; margin-top:12px; flex-wrap:wrap")}>
          <span style={css("font-size:10.5px; font-weight:700; letter-spacing:0.05em; text-transform:uppercase; color:#0A2240; background:#EAF0F8; border:1px solid #C4D0E0; padding:3px 7px; border-radius:2px")}>{v.roleLabel}</span>
        ...
        <button onClick={v.signOut} style={css("font:inherit; width:100%; font-size:13.5px; font-weight:700; cursor:pointer; padding:11px 14px; border:1px solid #0A2240; background:#fff; color:#0A2240; border-radius:3px")}>Sign out</button>
