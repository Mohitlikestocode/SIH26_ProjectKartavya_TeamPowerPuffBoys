    ...
    <>
      ...
      <div role="dialog" aria-label="Language and accessibility" style={css("position:fixed; top:92px; right:24px; width:min(92vw,320px); background:#fff; border:1px solid #C9CFD8; box-shadow:0 10px 30px rgba(10,34,64,0.20); z-index:65; border-radius:4px; padding:18px")}>
        <div style={css("display:flex; justify-content:space-between; align-items:baseline; gap:12px; margin-bottom:14px")}>
          <div style={css("font-size:14.5px; font-weight:700; color:#123E7C")}>Language &amp; accessibility</div>
          <button onClick={v.closePopovers} aria-label="Close" style={css("font:inherit; font-size:12px; font-weight:700; cursor:pointer; padding:5px 9px; border:1px solid #C9CFD8; background:#fff; color:#123E7C; border-radius:8px")}>Close</button>
        </div>
        ...
        <select id="langSel" onChange={v.onLangSelect} value={v.lang} style={css("width:100%; font:inherit; font-size:14px; padding:10px 11px; border:1px solid #C9CFD8; background:#fff; color:#1A1D23; border-radius:8px")}>
        ...
        <div style={css("display:flex")}>
          <button onClick={v.textDown} style={css(`font:inherit; flex:1; font-size:12.5px; font-weight:600; cursor:pointer; padding:10px 0; border:1px solid #C9CFD8; background:${v.sizeSmBg}; color:${v.sizeSmFg}; border-radius:8px 0 0 3px`)}>Small</button>
        ...
        <button onClick={v.toggleContrast} style={css(`font:inherit; width:100%; font-size:13.5px; font-weight:700; cursor:pointer; padding:11px 14px; border:1px solid ${v.contrastBorder}; background:${v.contrastBtnBg}; color:${v.contrastBtnFg}; border-radius:8px`)}>High contrast · {v.contrastState}</button>
