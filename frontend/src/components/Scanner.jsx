...
export default function Scanner({ v }) {
  return (
    <div role="dialog" aria-label="Scan session QR code" style={css("position:fixed; inset:0; background:#101318; z-index:80; display:flex; flex-direction:column")}>
      <div style={css("display:flex; justify-content:space-between; align-items:center; gap:16px; padding:14px 20px; background:#0A2240; color:#fff; flex-wrap:wrap")}>
        <div style={css("display:flex; align-items:center; gap:11px")}>
          <span style={css("width:30px; height:30px; border-radius:50%; background:#fff; border:2px solid #B08D2E; display:flex; align-items:center; justify-content:center")}>
            <span style={css("font-family:'Noto Sans Devanagari',sans-serif; font-size:15px; font-weight:700; color:#0A2240; line-height:1; margin-top:1px")}>क</span>
          </span>
          <div>
            <div style={css("font-size:14.5px; font-weight:700")}>Scan session QR code</div>
            <div style={css("font-size:11.5px; color:#9DB2CD")}>Point your camera at the code shown by your trainer</div>
          </div>
        </div>
        <div style={css("display:flex; gap:9px; align-items:center")}>
          <button onClick={v.openScanInfo} aria-label="How joining works" style={css("font:inherit; width:34px; height:34px; cursor:pointer; border:1px solid #3A567C; background:transparent; color:#fff; border-radius:50%; font-size:14px; font-weight:700")}>i</button>
          <button onClick={v.closeScanner} style={css("font:inherit; font-size:13px; font-weight:700; cursor:pointer; padding:9px 15px; border:1px solid #3A567C; background:transparent; color:#fff; border-radius:3px")}>Cancel</button>
        </div>
      </div>
      <div style={css("flex:1; min-height:0; position:relative; display:flex; align-items:center; justify-content:center; overflow:hidden; background:#15181E")}>
        ...
        <div style={css("position:relative; width:min(74vw,320px); max-width:80%; max-height:70%; aspect-ratio:1")}>
          ...
          <div style={css("position:absolute; left:6px; right:6px; top:50%; height:2px; background:#B08D2E; box-shadow:0 0 12px rgba(176,141,46,0.8)")}></div>
        </div>
      ...
      <div style={css("background:#fff; padding:18px 20px 22px")}>
        <div style={css("max-width:520px; margin:0 auto")}>
          <div style={css("display:flex; gap:10px; justify-content:center; margin-bottom:16px; flex-wrap:wrap")}>
            <button style={css("font:inherit; font-size:13px; font-weight:600; cursor:pointer; padding:10px 14px; border:1px solid #C9CFD8; background:#fff; color:#0A2240; border-radius:3px")}>Torch</button>
            <button style={css("font:inherit; font-size:13px; font-weight:600; cursor:pointer; padding:10px 14px; border:1px solid #C9CFD8; background:#fff; color:#0A2240; border-radius:3px")}>Switch camera</button>
            <button style={css("font:inherit; font-size:13px; font-weight:600; cursor:pointer; padding:10px 14px; border:1px solid #C9CFD8; background:#fff; color:#0A2240; border-radius:3px")}>Upload a photo of the code</button>
          </div>
          <div style={css("border-top:1px solid #EEF0F3; padding-top:16px")}>
            ...
            <div style={css("display:flex; gap:9px; flex-wrap:wrap")}>
              <input id="sessCode" type="text" placeholder="e.g. NSSTA-8841" style={css("flex:1; min-width:180px; font:inherit; font-size:15px; letter-spacing:0.06em; padding:12px 13px; border:1px solid #C9CFD8; border-radius:3px")} />
              <button style={css("font:inherit; font-size:14px; font-weight:700; cursor:pointer; padding:12px 20px; border:0; background:#0A2240; color:#fff; border-radius:3px")}>Join session</button>
            </div>
