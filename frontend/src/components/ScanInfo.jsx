      ...
      <div style={css("background:#fff; border:1px solid #C9CFD8; max-width:560px; width:100%; max-height:100%; overflow:auto")}>
        <div style={css("display:flex; justify-content:space-between; align-items:flex-start; gap:16px; padding:20px 22px 0")}>
          <div>
            ...
            <h2 style={css("font-family:'Poppins',sans-serif; font-size:21px; font-weight:700; color:#123E7C; margin:5px 0 0")}>How joining a session works</h2>
          </div>
          <button onClick={v.closeScanInfo} aria-label="Close" style={css("font:inherit; font-size:13px; font-weight:700; cursor:pointer; padding:8px 13px; border:1px solid #C9CFD8; background:#fff; color:#123E7C; border-radius:8px")}>Close</button>
        </div>
        <div style={css("padding:16px 22px 22px")}>
          {v.flowSteps.map((f) => (
            <div key={f.n} style={css("display:grid; grid-template-columns:28px 1fr; gap:13px; padding:11px 0; border-bottom:1px solid #EEF0F3")}>
              <div style={css("width:28px; height:28px; border-radius:50%; border:1px solid #B9CFEC; background:#E8F0FA; color:#123E7C; display:flex; align-items:center; justify-content:center; font-family:'IBM Plex Mono',monospace; font-size:12px")}>{f.n}</div>
              <div>
          ...
          <button onClick={v.closeScanInfo} style={css("font:inherit; width:100%; font-size:14px; font-weight:700; cursor:pointer; padding:12px 16px; border:0; background:#123E7C; color:#fff; border-radius:8px; margin-top:16px")}>Got it</button>
        </div>
      </div>
    </div>
  );
