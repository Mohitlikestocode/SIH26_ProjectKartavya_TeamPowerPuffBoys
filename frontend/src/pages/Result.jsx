...
export default function Result({ v }) {
  return (
    <section style={css("padding:24px 0 0; max-width:1100px; margin:0 auto")}>
      <div style={css("background:#fff; border:1px solid #D9DDE4; border-top:3px solid #166534; padding:24px 26px")}>
        <div style={css("display:flex; justify-content:space-between; gap:20px; align-items:flex-start; flex-wrap:wrap")}>
          <div>
            <div style={css("display:inline-flex; align-items:center; gap:8px; font-size:10.5px; font-weight:700; letter-spacing:0.08em; text-transform:uppercase; color:#166534; background:#EBF5EE; border:1px solid #BBDEC7; padding:3px 8px; border-radius:2px")}>Assessment complete</div>
            <h1 style={css("font-family:'Source Serif 4',serif; font-size:26px; font-weight:700; color:#0A2240; margin:10px 0 4px")}>Your results and instant feedback</h1>
          ...
          <div style={css("display:grid; gap:9px")}>
            <button onClick={v.goDash} style={css("font:inherit; font-size:14px; font-weight:700; cursor:pointer; padding:12px 20px; border:0; background:#0A2240; color:#fff; border-radius:3px; white-space:nowrap")}>See my gap map</button>
            <button onClick={v.goPath} style={css("font:inherit; font-size:14px; font-weight:600; cursor:pointer; padding:12px 20px; border:1px solid #C9CFD8; background:#fff; color:#0A2240; border-radius:3px; white-space:nowrap")}>Open my learning path</button>
          </div>
        </div>
        <div style={css("display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr)); gap:12px; margin-top:18px; padding-top:18px; border-top:1px solid #EEF0F3")}>
      ...
      <div style={css("display:flex; justify-content:space-between; align-items:baseline; gap:12px; margin:24px 0 12px; flex-wrap:wrap")}>
        <h2 style={css("font-family:'Source Serif 4',serif; font-size:21px; font-weight:700; color:#0A2240; margin:0")}>Item-by-item feedback</h2>
      ...
      <div style={css("display:grid; gap:12px")}>
        {v.review.map((r, i) => (
          <div key={i} style={css("background:#fff; border:1px solid #D9DDE4; padding:18px 20px")}>
            <div style={css("display:flex; gap:9px; align-items:center; flex-wrap:wrap; margin-bottom:9px")}>
              ...
              <span style={css(`font-size:10.5px; font-weight:700; padding:3px 7px; border-radius:2px; color:${r.domColor}; background:${r.domTint}; border:1px solid ${r.domBorder}`)}>{r.domain}</span>
              <span style={css("font-size:11.5px; color:#7A8492")}>{r.skill}</span>
              <span style={css(`margin-left:auto; font-size:11.5px; font-weight:700; padding:4px 9px; border-radius:2px; color:${r.verdictColor}; background:${r.verdictBg}`)}>{r.verdict}</span>
            </div>
            <div style={css("font-size:14.5px; font-weight:600; color:#1A1D23; line-height:1.45; max-width:820px")}>{r.stem}</div>
            <div style={css("display:grid; grid-template-columns:repeat(auto-fit,minmax(260px,1fr)); gap:12px; margin-top:12px")}>
            ...
            <div style={css("font-size:13px; color:#3B424E; line-height:1.6; margin-top:12px; border-left:2px solid #B08D2E; padding-left:12px; max-width:860px")}><strong style={css("color:#0A2240")}>Why.</strong> {r.explain}</div>
          </div>
        ))}
      </div>
    </section>
