      ...
      {v.assessed && (
        <div>
          <div style={css("display:flex; justify-content:space-between; align-items:flex-end; gap:20px; flex-wrap:wrap; margin-bottom:18px")}>
            <div>
              <h1 style={css("font-family:'Poppins',sans-serif; font-size:28px; font-weight:700; color:#123E7C; margin:0")}>Your learning path</h1>
          ...
          <div style={css("background:#fff; border:1px solid #E3E9F2; padding:20px 24px; margin-bottom:20px")}>
              ...
              {v.stageHeads.map((s, i) => (
                <div key={i} style={css("display:grid; grid-template-columns:38px 1fr; gap:12px; align-items:start")}>
                  ...
                  <div>
                    ...
                    <div style={css("font-family:'Poppins',sans-serif; font-size:18px; font-weight:700; color:#123E7C; margin-top:2px; line-height:1.25")}>{s.title}</div>
          ...
          <div style={css("display:grid; gap:20px")}>
            {v.stages.map((st, si) => (
              <div key={si} style={css(`background:#fff; border:1px solid #E3E9F2; border-left:4px solid ${st.accent}; padding:22px 24px`)}>
                <div style={css("display:flex; align-items:center; gap:12px; flex-wrap:wrap; padding-bottom:14px; border-bottom:1px solid #EEF0F3; margin-bottom:16px")}>
                  ...
                  <div>
                    <div style={css("font-family:'Poppins',sans-serif; font-size:19px; font-weight:700; color:#123E7C; line-height:1.2")}>{st.title}</div>
                    <div style={css("font-size:12px; color:#7A8492; margin-top:2px")}>{st.gate}</div>
                  </div>
                </div>
                <div style={css("display:grid; grid-template-columns:repeat(auto-fit,minmax(min(100%,300px),1fr)); gap:14px")}>
                  {st.items.map((it, ii) => (
                    <div key={ii} className="path-item" style={css(`border:1px solid #E3E9F2; border-top:3px solid ${it.srcColor}; background:#FCFCFD; padding:16px; display:flex; flex-direction:column; gap:10px`)}>
                      <div style={css("display:flex; justify-content:space-between; align-items:center; gap:8px; flex-wrap:wrap")}>
                        <span style={css(`display:inline-flex; align-items:center; gap:6px; border:1px solid ${it.srcBorder}; background:${it.srcTint}; padding:3px 7px; border-radius:6px; font-size:10.5px; font-weight:700; color:${it.srcFg}; white-space:nowrap`)}><span style={css(`width:7px;height:7px; background:${it.srcColor}`)}></span>{it.source}</span>
                        <span style={css("font-family:'IBM Plex Mono',monospace; font-size:10.5px; color:#7A8492")}>{it.match} match</span>
                      </div>
                      <div style={css("font-size:14.5px; font-weight:700; color:#123E7C; line-height:1.35")}>{it.title}</div>
                      ...
                      <button className="path-item-cta" style={css("font:inherit; font-size:13px; font-weight:700; cursor:pointer; padding:9px 12px; border:1px solid #123E7C; background:#fff; color:#123E7C; border-radius:8px")}>{it.cta}</button>
                    </div>
                  ))}
                </div>
              </div>
