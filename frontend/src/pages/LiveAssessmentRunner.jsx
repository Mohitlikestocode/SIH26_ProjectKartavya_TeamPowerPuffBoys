...
// renders whatever Assessment.questions the backend actually returned for
// a real Attempt, submits real answers, and — when the assessment is
// isProctored — mounts the real ProctoringHarness alongside it.
export default function LiveAssessmentRunner({ assessment, attemptId, token, onSubmitted }) {
  ...
  return (
    <section style={css("padding:24px 0 0; max-width:1180px; margin:0 auto")}>
      <div style={css("display:flex; justify-content:space-between; align-items:flex-end; gap:16px; flex-wrap:wrap; margin-bottom:14px")}>
        ...
        <div style={css("font-size:12.5px; color:#5A6472; text-align:right")}>
          Item {cursor + 1} of {questions.length} · {answeredCount} answered
          {assessment.isProctored && <><br />🔴 Proctored</>}
        </div>
      </div>
